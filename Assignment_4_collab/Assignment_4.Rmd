...
Which similarity score sums are the most frequent?

```{r}
sum_cols <- colSums(model_info$sim > 0)
qplot(sum_cols, fill = I("steelblue"), col = I("#02ff35")) +
...
that will be recommended to each user. Note that we will be using the testing set here.
...
films from our training set; to visualize, among top 20, which ones are most
similar to which other movies. This is exploring what will be the basis of our
recommendations (recommending movies similar to what a user already liked).
...
From the `recommendation_matrix` that we generated above, we can visualize the 
distribution (by count) of the top 10 recommended movie titles via this 
histrogram. 
...
ggplot(recommendation_table_top[1:10, ], aes(x = title, y = n)) +
    geom_bar(stat = "identity", show.legend = FALSE, width = 0.8) +
    labs(x = "Movie Title",
         y = "Frequency",
         title = "Distribution of recommendation matrix") +
...
So we are seeing the most frequently recommended movies is 
